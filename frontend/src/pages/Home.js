...
export const Home = () => {
  ...
  useEffect(() => {
    window.onscroll = () => {
      setParalaxPosition(
        paralaxref.current?.clientHeight
          ? (window.pageYOffset / paralaxref.current.clientHeight) *
              -window.innerHeight
          : 0
      );
  ...
  return (
    <Box ref={paralaxref} className={classes.foregroundStyle}>
      <CardMedia
        ...
        component={"img"}
        image={"https://i.imgur.com/S919SiU.png"}
        alt="fill murray"
      />
      <div style={{ titleStyle }}>
        ...
        >
          <Typography
            variant="h4"
            style={{ textAlign: "center", transform: "translateY(1rem)" }}
          >
            Choose your
          </Typography>
          ...
          <Typography variant="body2" style={{ textAlign: "center" }}>
            With more than 140 champions, you’ll find the perfect match
            <br /> for your playstyle. Master one, or master them all.
          </Typography>
        </Typography>
      </div>
        ...
        >
          {filteredRoutes.map((route, index) => (
            <Grid
              ...
              xl={3}
              onClick={() => {
                window.location.href = `${process.env.PUBLIC_URL}${route.path}`;
              }}
              key={index}
            >
