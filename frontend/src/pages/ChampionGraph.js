...
export const ChampionGraph = () => {
  ...
  const [arrayOfChampionsNamesAndIndex, setArrayOfChampionsNamesAndIndex] =
    useState([]);

  useEffect(() => {
    Papa.parse(`${process.env.PUBLIC_URL}/sheet.csv`, {
      download: true,
  ...
  const findChampionIndex = useCallback(
    (x) => {
  ...
  useEffect(() => {
      ...
      [
        {
          ...
          data: championDataObject[
            arrayOfChampionsNamesAndIndex.find(findChampionIndex).value + 1
          ]?.slice(1, 10),
        },
      ],
      [championDataObject]
    ...
    setPic(
      `https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${location}_0.jpg`
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  });

  const graphData = {
    labels: ["H", "H/Lv", "M", "M/Lv", "AD", "AS", "A", "MR", "R", "MS"],
    datasets: datasets,
  };
